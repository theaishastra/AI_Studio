...
function submitBooking(){
  ...
  /* hand the whole booking off to a dedicated confirmation page instead of showing
     the success state inline — this page (product + form) is left untouched so
     hitting Back always lands on a clean, blank booking form */
  const q=new URLSearchParams({category:cat,package:current.pkg,tier:current.tier,price:current.price,name,phone,email,venue,msg});
  if(!usesEventSchedule){ q.set("date",date); q.set("time",time); }
  if(eventSchedule.length){ q.set("events",eventSchedule.join("||")); }
  if(selectedEquipSet.size > 0){
    const selectedList = Array.from(selectedEquipSet).map(id => EQUIPMENT_DATA[id].name);
    q.set("equipments", selectedList.join(", "));
  }
  window.location.href="booking-confirmation.html?"+q.toString();
  /* TODO: send to your backend / Supabase / WhatsApp here */
}
        ...
        <div class="modal-gallery-thumbs">
          ${item.gallery.map((img, idx) => `
            <div class="modal-thumb ${idx === 0 ? 'active' : ''}" onclick="setModalMainImg('${img}', this)">
              <img src="${cldOpt(img)}" alt="Thumbnail ${idx + 1}">
            </div>
          `).join('')}
        </div>
      </div>
